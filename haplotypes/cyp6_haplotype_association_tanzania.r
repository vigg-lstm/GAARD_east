# Since we know that the Cyp6 region is important, and that there are sweeps and CNVs there, 
# we can associate sweep and CNVs with resistance at this locus. 
...
study.pops <- c('Moshi_arabiensis_Delta',
...
cyp6.range <- c(28462725, 28500939)
window.range <- paste('2R:', cyp6.range[1], '-', cyp6.range[2], sep = '')

sig.test.list <- list()
for (pop in study.pops){
	assign.hap.command <- paste(python.env, 'assign_haplotypes_tanzania.py', pop, window.range, '..', 
	                            paste(pop, 'Cyp6.csv', sep = '_')
	                      )
	system(assign.hap.command)
	# Load the results
	haplotypes <- fread(paste(pop, '_Cyp6.csv', sep = ''), sep = '\t', key = 'sample_name')
	...
	sig.test[, window := 'Cyp6']
	sig.test.list[[pop]] <- sig.test
}
sig.tests <- rbindlist(sig.test.list, idcol = 'population')

fwrite(sig.tests, file = 'haplotype_significance_tests_tanzania_Cyp6.csv', sep = '\t')
...
plot.cyp6.hap.cluster <- function(pop.win, filename, point.cex, remove.background.snps = T, add = F){
	chrom <- '2R'
	start.pos <- cyp6.range[1]
	end.pos <- cyp6.range[2]
	# Get the SNPs
...
for(pop.win in names(snp.tables))
	plot.cyp6.hap.cluster(pop.win, paste(pop.win, '.png', sep = ''), point.cex = 0.8)
