...
meta = pd.merge(
    pd.read_csv(
        repo_folder + '/data/tanzania.samples.meta.csv',
        sep = '\t',
        index_col = 'partner_sample_id'
    ).drop(['location', 'country'], axis = 1),
    pd.read_csv(
        repo_folder + '/data/sample_phenotypes_EA.csv',
        sep = '\t',
        index_col = 'specimen'
    ),
    left_index = True,
...
site_filter = ag3.snp_calls(region=region, sample_sets="3.7")['variant_filter_pass_arab']
