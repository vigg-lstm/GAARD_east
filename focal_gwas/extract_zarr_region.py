...
if len(argv) == 6:
	...
	raise Exception('Fail. There should be 5 or 7 command line arguments (zarr_folder, study_id, chrom, region, output_fn (, sample_names, meta_fn)).')
...
if sample_names != 'all':
	meta = pd.read_csv(meta_fn, sep = '\t', index_col = 'partner_sample_id')
	sample_names = sample_names.split(',')
	samples_zarr = zarr_folder + '/' + study_id + '/samples'
	# Get the sample names
	malgen_IDs = [sub('-.*', '', x) for x in meta.loc[sample_names, 'sample_id']]
	# Convert the sample names to Malgen IDs
	malgen_sample_names = [sub('-.*', '', x.decode('utf-8')) for x in zarr.open(os.path.expanduser(samples_zarr), mode = 'r')]
	# Get the indices of the vobs sample names that match the malgen_IDs
	sample_boolean = np.isin(np.array(malgen_sample_names), malgen_IDs)
