...
h12.p.thresh <- 0.01
focal.window.ranges <- h12.table[is.peak == T & pval < c(h12.p.thresh)] %>%
...
# Identify males
meta <- fread('../../data/tanzania.samples.meta.csv')
...
# quality filters. This is in fact unecessary since these samples have already been removed,
# but we include this bit of code as a safety check. 
...
# Remove sibs, males and samples to exclude
males.to.remove <- intersect(names(all.snps), males)
samples.to.remove <- c(sibs.to.remove, males.to.remove, samples.to.exclude)
all.snps[, c(samples.to.remove) := NULL]
sample.names <- grep('EA18-\\d{4}', names(all.snps), value = T)
# Focus on focal regions
extract.window.snps <- function(window.name, region.border.size = 10000){
	all.snps[Chrom == focal.window.ranges[window.name, chromosome] & 
	         Pos >= (focal.window.ranges[window.name, startpoint] - region.border.size) &
	         Pos <= (focal.window.ranges[window.name, endpoint] + region.border.size)] %>%
	.[, Window.name := window.name] %>%
	# Record whether the SNP is in the focal window, on in the border
	.[, In.window := Pos >= focal.window.ranges[window.name, startpoint] & Pos <= focal.window.ranges[window.name, endpoint]]
}
...
# Load phenotype data
phenotype.filename <- '../../data/sample_phenotypes_EA.csv'
...
sample.name.conversion.file <- '../../data/tanzania.samples.meta.csv'

get.allele.counts <- function(window.name, region.border.size = 10000){
	chrom <- focal.window.ranges[window.name, chromosome]
	window.start <- max(0, focal.window.ranges[window.name, startpoint] - region.border.size)
	window.end <- min(chrom.sizes[chrom], focal.window.ranges[window.name, endpoint] + region.border.size)
...
snp.eff.command <- paste(
	'/usr/bin/java -jar ~/software/snpEff/snpEff.jar eff Anopheles_gambiae', 
	'-noStats', 
	vcf.file, 
	'>', 
	vcf.output.file
...
plot.focal.window <- function(window.name, filename, sig.snps.only = T, true.pos = T, region.border.size = 10000, segment.lwd = 0.2, ...){
	chrom <- focal.window.ranges[window.name, chromosome]
	start.pos <- max(0, focal.window.ranges[window.name, startpoint] - region.border.size)
	end.pos <- min(chrom.sizes[chrom], focal.window.ranges[window.name, endpoint] + region.border.size)
