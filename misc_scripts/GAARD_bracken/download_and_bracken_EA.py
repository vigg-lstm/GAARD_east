...
# Adapted from download_and_bracken3.py for East Africa samples
...
accession_table_fn = '~/data/ML/GAARD_bracken/data/sample_accessions_EA.csv'
accession_table = pd.read_csv(accession_table_fn, sep = '\t', names = ['GAARD_SampleID', 'Country', 'Location', 'MalGEN_ID', 'Date_of_collection', 'StudyID', 'Run_accession', 'Fastq1', 'Fastq2'], index_col = 'GAARD_SampleID')
...
for s in sequenced_sample_names:
	...
	download_command = '~/data/ML/GAARD_bracken/download_and_bracken_GAARD_sample_EA.sh ' + s + ' ' + accession_table_fn + ' /dev/shm/kraken_database ' + path_to_db + ' ' + threads
