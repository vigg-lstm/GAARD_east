...
if (len(argv) == 5):
	...
	if species == 'arabiensis':
		...
		site_filter = 'gamb_colu'
	conditions = 'location == "' + location + '" & species == "' + species + '" & insecticide == "' + insecticide + '"'
else:
	raise Exception("""Fail. There should be four or five command line arguments.
                       With four arguments: (root_folder, study_id, site_filter, conditions).
...
print('\tsite_filter = ' + site_filter)
print('\tconditions = ' + conditions + '\n')
stdout.flush()


# Load the metadata and sample name conversion table
meta_fn = '../../data_east/sample_metadata_EA.csv'
...
phenotypes_fn = '../../data_east/sample_phenotypes_tanzania.csv'
...
accessibility_fn = root_folder + '/site_filters/dt_20200416/' + site_filter
...
def load_and_filter(zarr_path, chrom, verbose = False):
	...
	mutsum_df = pd.concat([pd.DataFrame([chrom]*len(filtered_variant_pos), columns = ['Chrom']), pd.DataFrame(filtered_variant_pos, columns = ['Pos']), pd.DataFrame(mutsum_variants_for_output, columns = gaard_sample_names)], axis = 1)
	mutsum_df_acc = pd.concat([pd.DataFrame([chrom]*len(filtered_variant_pos_acc), columns = ['Chrom']), pd.DataFrame(filtered_variant_pos_acc, columns = ['Pos']), pd.DataFrame(mutsum_variants_for_output_acc, columns = gaard_sample_names)], axis = 1)
	# Pruned mutsum tables
	mutsum_df_pruned = pd.concat([pd.DataFrame([chrom]*np.sum(LD_prune), columns = ['Chrom']), pd.DataFrame(filtered_variant_pos.compress(LD_prune), columns = ['Pos']), pd.DataFrame(mutsum_variants_for_output.compress(LD_prune, axis = 0), columns = gaard_sample_names)], axis = 1)
	mutsum_df_acc_pruned = pd.concat([pd.DataFrame([chrom]*np.sum(LD_prune_acc), columns = ['Chrom']), pd.DataFrame(filtered_variant_pos_acc.compress(LD_prune_acc), columns = ['Pos']), pd.DataFrame(mutsum_variants_for_output_acc.compress(LD_prune_acc, axis = 0), columns = gaard_sample_names)], axis = 1)
	# Unpruned presence / absence tables
	presabs_df = pd.concat([pd.DataFrame([chrom]*len(filtered_variant_pos), columns = ['Chrom']), pd.DataFrame(filtered_variant_pos, columns = ['Pos']), pd.DataFrame(presabs_variants_for_output, columns = gaard_sample_names)], axis = 1)
	presabs_df_acc = pd.concat([pd.DataFrame([chrom]*len(filtered_variant_pos_acc), columns = ['Chrom']), pd.DataFrame(filtered_variant_pos_acc, columns = ['Pos']), pd.DataFrame(presabs_variants_for_output_acc, columns = gaard_sample_names)], axis = 1)
	# Pruned presence / absence tables
	presabs_df_pruned = pd.concat([pd.DataFrame([chrom]*np.sum(LD_prune), columns = ['Chrom']), pd.DataFrame(filtered_variant_pos.compress(LD_prune), columns = ['Pos']), pd.DataFrame(presabs_variants_for_output.compress(LD_prune, axis = 0), columns = gaard_sample_names)], axis = 1)
	presabs_df_acc_pruned = pd.concat([pd.DataFrame([chrom]*np.sum(LD_prune_acc), columns = ['Chrom']), pd.DataFrame(filtered_variant_pos_acc.compress(LD_prune_acc), columns = ['Pos']), pd.DataFrame(presabs_variants_for_output_acc.compress(LD_prune_acc, axis = 0), columns = gaard_sample_names)], axis = 1)
...
mutsum_df = pd.concat([x[0] for x in all_variants_list], axis = 0)
mutsum_df_acc = pd.concat([x[1] for x in all_variants_list], axis = 0)
mutsum_df_pruned = pd.concat([x[2] for x in all_variants_list], axis = 0)
mutsum_df_acc_pruned = pd.concat([x[3] for x in all_variants_list], axis = 0)
presabs_df = pd.concat([x[4] for x in all_variants_list], axis = 0)
presabs_df_acc = pd.concat([x[5] for x in all_variants_list], axis = 0)
presabs_df_pruned = pd.concat([x[6] for x in all_variants_list], axis = 0)
presabs_df_acc_pruned = pd.concat([x[7] for x in all_variants_list], axis = 0)
