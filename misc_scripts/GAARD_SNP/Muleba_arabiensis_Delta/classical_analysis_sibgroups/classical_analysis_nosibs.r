...
study.pop <- 'Muleba_arabiensis_Delta'
study.id.table <- fread('../../data_east/study_ids.csv', key = 'population')
study.id <- study.id.table[study.pop, study_id]
sample.name.conversion.file <- '../../data_east/sample_metadata_tanzania.csv'
...
sib.groups <- fread('~/data/GAARD_repo/GAARD_east/NGSrelate/full_relatedness_tanzania/sib_group_table.csv', sep = '\t')
...
# Load phenotype data
phenotype.filename <- '~/data/ML/GAARD_SNP/data_east/sample_phenotypes_EA.csv'
...
# There is 1 significant SNP after FDR correction, which is negative correlated with resistance
fwrite(sig.snps, 'fdr_significant_snps.csv', sep = '\t')
...
clump.window.size = 100000
top.snps$approx.pos <- paste(top.snps$Chrom, round(top.snps$Pos / clump.window.size), sep = ':')
...
gff.path <- '../../data_east/VectorBase-57_AgambiaePEST.gff'
...
# Get clumps with at least 10 SNPs and merge adjacent ones
big.clumps <- snp.clumps[snp.clumps >= 10] %>% 
			  data.table() %>%
			  setnames('V1', 'Clump') %>%
			  .[, Chrom := sub(':.*', '', Clump)] %>%
			  .[, Pos := as.integer(sub('.*:', '', Clump))] %>%
			  .[order(Chrom, Pos)] %>%
              .[, clump.id := (Chrom != c('.', Chrom[-.N]) | Pos != c(-2, Pos[-.N]) + 1) %>%
			                  cumsum() %>%
							  paste('clump', ., sep = '')
			  ]

# Write a function to investigate a clump
search.clump <- function(clump.info){
	chrom <- unique(clump.info$Chrom)
	pos <- clump.info$Pos
	clump <- list(clump.id = unique(clump.info$clump.id),
				  snp.ids = as.character(top.snps[approx.pos %in% paste(chrom, pos, sep = ':'), ][order(Pos), ]$ID),
	              chrom = chrom,
	              start = (pos[1] - 0.5) * clump.window.size,
	              end = (pos[length(pos)] + 0.5) * clump.window.size
	         )
	clump$genes <- unique(str_extract(subset(gff, (seqid == chrom) & 
	                                              (start < clump$end) & 
	                                              (end > clump$start) &
	                                              type == 'protein_coding_gene')$attributes,
	                                              'AGAP\\d{6}'
						  )
                   )
	clump
...
clumps <- lapply(split(big.clumps, by = 'clump.id'), search.clump)
...
get.allele.counts <- function(clump){
	...
	system(paste('~/miniconda3/envs/mozzie37/bin/python ../../extract_zarr_region_v2.py', 
	                                                    '~/scratch/VObs_GAARD',
														study.id,
														clump$chrom,
														window.range, 
