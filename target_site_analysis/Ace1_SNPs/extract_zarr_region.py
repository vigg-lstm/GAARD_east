...
if len(argv) == 6:
	...
	sample_names = argv[6]
	meta_fn = argv[7]
	vobs_id_column = argv[8]
	gaard_id_column = argv[9]
else:
	raise Exception('Fail. There should be 5 or 9 command line arguments (zarr_folder, study_id, chrom, region, output_fn (, sample_names, meta_fn, vobs_id_column, gaard_id_column)).')
...
if sample_names != 'all':
	meta = pd.read_csv(meta_fn, sep = '\t', index_col = gaard_id_column)
