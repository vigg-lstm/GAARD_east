...
phen <- fread('../data/sample_phenotypes_EA.csv', key = 'specimen')
...
zarr.folders <- c(Tanzania = '~/scratch/VObs_GAARD/1246-VO-TZ-KABULA-VMF00185')
...
kdr.genotypes <- lapply(zarr.folders, get.zarr.genotypes, '2L', kdr.indices) %>%
                 ...
                 .[sort(rownames(.)), sort(colnames(.)), drop = F] %>%
				 .[, !(colnames(.) %in% samples.to.remove), drop = F]
...
fwrite(t.pop.freqs, 'target_site_genotype_frequencies_tanzania.csv', sep = '\t')
...
# Let's test things in Moshi Delta
cat('\n\nMoshi Delta:\n')
moshi.delta.seg.markers <- study.freqs[.('Moshi', 'Delta'), ..all.snps] %>%
                           {(. >= 0.1) & (. <= 0.9)} %>%
                           colnames(.)[.]
moshi.delta.table <- as.data.frame(wgs.phen[.('Moshi', 'Delta')])
glm.up(moshi.delta.table, moshi.delta.seg.markers, 'phenotype')
cat('\nAll markers too rare\n')

# Muleba Delta
cat('\n\nMuleba Delta:\n')
muleba.delta.seg.markers <- study.freqs[.('Muleba', 'Delta'), ..all.snps] %>%
                             {(. >= 0.1) & (. <= 0.9)} %>%
                             colnames(.)[.]
muleba.delta.table <- as.data.frame(wgs.phen[.('Muleba', 'Delta')])
glm.up(muleba.delta.table, muleba.delta.seg.markers, 'phenotype')
cat('\nAll markers too rare.\n')

# Moshi PM
cat('\n\nMoshi PM:\n')
moshi.pm.seg.markers <- study.freqs[.('Moshi', 'PM'), ..all.snps] %>%
                          {(. >= 0.1) & (. <= 0.9)} %>%
                          colnames(.)[.]
moshi.pm.table <- as.data.frame(wgs.phen[.('Moshi', 'PM')])
glm.up(moshi.pm.table, moshi.pm.seg.markers, 'phenotype')
cat('\nAll markers too rare.\n')

# Now let's try combining locations:
delta.table <- as.data.frame(wgs.phen[insecticide == 'Delta'])
cat('\n\nAll populations Delta:\n')
glm.up(delta.table, all.snps, 'phenotype', control.for = 'location', glm.function = 'glmmTMB')
...
output.table <- setnames(wgs.phen[, !c('country', 'concentration', 'exposure_time', 'plate')],
                         'specimen', 'sample.id')
fwrite(output.table, 'target_site_genotypes_tanzania.csv', sep = '\t')
